...
function tesseract_get_packages_url() {
	...
	/* FOR SERVER 
	$packages_url = 'https://s3.amazonaws.com/tesseracttheme/packages_designer.json';
	...
	*/
	...
	/* FOR LOCAL */
	$content = file_get_contents(get_template_directory_uri().'/packages_designer.json',0,null,null);
...
function tesseract_import_packages( $packages ) {
	...
	//echo "<pre>"; print_r($packages);echo "</pre>"; die;
	if ( empty( $packages ) ) {
		...
		foreach ( $posts as $post ) {
			...
			if ( is_wp_error( $post_id ) ) {
				...
				if ( in_array( $post['post_type'], Tesseract_Importer_Constants::$TRACKED_POST_TYPES ) ) {
					...
					/*if(term_exists($post['tax_input'][0],'fl-builder-template-category')){
						print_r(wp_set_object_terms( $post_id, $post['tax_input'][0], 'fl-builder-template-category' ));
					}*/
					if(array_key_exists('tax', $post) && count($post['tax'])!=0){
						for($i=0;$i<=count($post['tax']);$i++)
						{
							if(!term_exists($post['tax'][$i],'fl-builder-template-category'))
							{
								wp_insert_term($post['tax'][$i],'fl-builder-template-category');
							}
							wp_set_object_terms( $post_id, $post['tax'][$i], 'fl-builder-template-category' );
							unset($post['tax']);
							break;
						}
					}
