<?php
 class Menu_Font_Styles{
	public static function register( $wp_customize ) {
		...
		$wp_customize->add_setting( 'header_menu_spacing',
			array(
				'default' => '10',
				'transport' => 'refresh'
			)
		...
		$wp_customize->add_control( 
			new WP_Customize_Control(
				...
				'header_menu_spacing',
				array(
					'label'          => __( 'Menu Item Spacing(In px.)', 'tesseract' ),
					'section'        => 'header_menu_font_styles',
					'settings'       => 'header_menu_spacing',
					'type'           => 'select',
					'choices' => array('10'=>'10','15'=>'15','20'=>'20','25'=>'25','30'=>'30','35'=>'35','40'=>'40','45'=>'45','50'=>'50'),						
				)
			)
		);
		...
		/*$wp_customize->add_setting( 'footer_menu_spacing',
			array(
				'default' => 12,
				'transport' => 'refresh'
			)
		...
		$wp_customize->add_section( 'tesseract_footer_centre', array(
			...
			'title'      => __('Footer Center Block Content', 'tesseract-remove-branding'),
			...
			$choices = array(
				...
				/*'menuhtml' => __('Menu and HTML', 'tesseract')*/
		...
		if ( empty( $footer_centre_content_menu_selector_menus ) ||  !$menus['secondary'] ) {
			...
			$footer_centre_content_menu_selector_items = array( 'none' => "You haven't made/assign any menus!" );
	...
	public static function header_output() 
	{
		...
		$color = (get_theme_mod('tesseract_header_colors_link_color')) ? get_theme_mod('tesseract_header_colors_link_color') : '#000000';
		...
		$link_color = (get_theme_mod('tesseract_header_colors_link_color')) ? get_theme_mod('tesseract_header_colors_link_color') : '#000000';
		...
		$tesseract_header_activated_link_color = (get_theme_mod('tesseract_header_activated_link_color')) ? get_theme_mod('tesseract_header_activated_link_color') : '#8BB6C7';
		...
		$sub_menu_color = (get_theme_mod('sub_menu_color')) ? get_theme_mod('sub_menu_color') : '#000000';
		
		$tesseract_header_colors_link_hover_color = (get_theme_mod('tesseract_header_colors_link_hover_color')) ? get_theme_mod('tesseract_header_colors_link_hover_color') : '#d1ecff';
		$header_menu_spacing = (get_theme_mod('header_menu_spacing')) ? get_theme_mod('header_menu_spacing') : '10';
	//echo "--------------------------------".$link_color;
		...
		<style type='text/css'>
			...
			{
				color: <?php echo $tesseract_header_activated_link_color ; ?> !important;
			}
			...
			.top-navigation li.current-menu-item.current_page_item ul li a{ color:<?php echo $sub_menu_color; ?> !important;}	
			.top-navigation li.current-menu-item.current_page_item ul li a:hover{color:<?php echo $tesseract_header_colors_link_hover_color; ?> !important;}
			...
			.top-navigation ul li > a{
					padding-right:<?php echo $header_menu_spacing.'px !important'; ?> ;
					padding-left:<?php echo $header_menu_spacing.'px !important'; ?> ;
				}
			#header-right-menu ul li > a{
					padding-right:<?php echo $header_menu_spacing.'px !important'; ?> ;
					padding-left:<?php echo $header_menu_spacing.'px !important'; ?> ;
			...
			.top-navigation ul li > a{
					font-size:<?php echo get_theme_mod('header_menu_text_size_custom').'px'; ?> !important ;
				}
			#header-right-menu ul li > a{
					font-size:<?php echo get_theme_mod('header_menu_text_size_custom').'px'; ?> !important ;
				}
			...
			#footer-right-menu ul li > a{
					padding-right:<?php echo get_theme_mod('footer_menu_spacing').'px'; ?> ;
				}
			#footer-horizontal-menu ul li > a{
					padding-right:<?php echo get_theme_mod('footer_menu_spacing').'px'; ?> ;
				}
	...
	public static function footer_centre_content()
	{
		...
		$content_default_html_centre = (get_theme_mod('tesseract_footer_centre_content_html')) ? get_theme_mod('tesseract_footer_centre_content_html') : '<strong>Theme by <a href="http://tesseracttheme.com">Tesseract</a></strong><strong><a href="http://tesseracttheme.com"> &nbsp;&nbsp;&nbsp;<img src="http://tylers-storage.s3-us-west-1.amazonaws.com/wp-content/uploads/2015/09/07185505/Drawing1.png" alt="Drawing" width="16" height="16" /></a></strong>';
					...
					<?php switch( $content_centre ) 
						{
							...
							$footer_centre_menu  = (get_theme_mod("tesseract_footer_centre_menu_select")) ? get_theme_mod("tesseract_footer_centre_menu_select") : 'none';
								if($footer_centre_menu != 'none')
								{
									...
									?>
                                    <div id="cetre-collaps" class="mobile-menu-collaps-button foot-collaps">MENU <i class="fa fa-bars"></i></div>

									<section id="footer-horizontal-menu" class="menu-centre cf <?php echo $footerNav_class; ?>"><?php wp_nav_menu(array('menu'=>$footer_centre_menu,'depth' => 1));//( FALSE, FALSE, 'Footer101', 1 ); ?></section>
									<?php
								}
							break;
							case 'logo':
	...
	public static function load_google_fonts()
	{
			...
			wp_register_style('googleFontsHeader','https://fonts.googleapis.com/css?family='.$header_fonts.':400,700');
   			wp_enqueue_style('googleFontsHeader');

   			$header_text_fonts = str_replace(" ","+",get_theme_mod('tesseract_header_logo_text_fonts'));
			wp_register_style('googleFontsHeader2','https://fonts.googleapis.com/css?family='.$header_text_fonts.':400,700');
   			wp_enqueue_style('googleFontsHeader2');


   			$footer_fonts = str_replace(" ","+",get_theme_mod('footer_menu_text_fonts'));
			wp_register_style('googleFontsFooter','https://fonts.googleapis.com/css?family='.$footer_fonts.':400,700');
			...
			wp_register_style('googleFontsHeader','https://fonts.googleapis.com/css?family='.$header_fonts);
   			...
   			$logo_font = (get_theme_mod('tesseract_header_logo_text_fonts')) ? get_theme_mod('tesseract_header_logo_text_fonts') : 'Pacifico';
   			wp_register_style('googleFontsLogoHeader','https://fonts.googleapis.com/css?family='.$logo_font);
   			wp_enqueue_style('googleFontsLogoHeader');
...
function tesseract_header_upper_html($location)
{
	// if(get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' )=='' || get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' )=='none' )
	// {
	// 	return false;
	// }
	...
	<div class="header-upper-<?php echo $location; ?>">
		<?php
			if($location == 'left')
			{
				$header_upper_content_type_choice =  (get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' )) ? get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' ) : 'text';

				$texts = (get_theme_mod('tesseract_header_upper_'.$location.'_content_text')) ? get_theme_mod('tesseract_header_upper_'.$location.'_content_text') : 'Text On Upper Header Left Panel';
			}
			if($location == 'right')
			{
				$header_upper_content_type_choice =  (get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' )) ? get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' ) : 'searchcurrency';
				$texts = '';
			}
			if($location == 'centre')
			{
				$header_upper_content_type_choice =  (get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' )) ? get_theme_mod( 'tesseract_header_upper_'.$location.'_content_type' ) : 'cartwoolink';
				$texts = '';
			}

			//echo '>>> '.$header_upper_content_type_choice;
			
			switch($header_upper_content_type_choice)
			{
				default:
				break;
				case 'text':
				echo '<div style="color:'.$tesseract_header_upper_text_color.'" class="'.$location.'-text">'.strip_tags($texts).'</div>';
				break;
				case 'social': ?>
					<ul class="hr-social">
						<?php 
				...
				<?php break;
				case 'textsocial':
					echo '<div style="color:'.$tesseract_header_upper_text_color.'" class="'.$location.'-text">'.strip_tags(get_theme_mod('tesseract_header_upper_'.$location.'_content_text')).'</div>';
					?>
					<ul class="hr-social">
						<?php 
							$bln_tesseract_social_account_right = false;
				...
				case 'cartwoolink':
					if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) {
						...
						?>
							<span class="cart-left" style="color:<?php echo $tesseract_header_upper_text_color;?>;">
								<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" class="cart-contents" href="<?php echo wc_get_cart_url(); ?>" title="<?php _e( 'View your shopping cart','tesseract' ); ?>">
									<?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?> - <?php echo WC()->cart->get_cart_total(); ?>
								</a>
								</span> <span class="divide-cart" style="color:<?php echo $tesseract_header_upper_text_color;?>;" >/ </span>
							<span class="cart-right" style="color:<?php echo $tesseract_header_upper_text_color;?>;">
								<?php if ( is_user_logged_in() ) { ?>
								 	<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('My Account','tesseract'); ?>"><?php _e('My Account','tesseract'); ?></a> or 
								 	<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" href="<?php echo site_url();?>/logout"><?php _e('Logout','tesseract'); ?></a>
								 <?php } 
								 else { ?>
								 	<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('Login / Register','tesseract'); ?>"><?php _e('Login / Register','tesseract'); ?></a> or
								 	<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" href="<?php echo $woocommerce->cart->get_checkout_url();?>" title="<?php _e('Checkout','tesseract'); ?>"><?php _e('Checkout','tesseract'); ?></a>
								<?php } ?>
							</span>
							<?php
				...
				case 'cartsummary':
					...
					?>
						...
						<span style="color:<?php echo $tesseract_header_upper_text_color;?>;"><a style="color:<?php echo $tesseract_header_upper_text_color;?>;" class="cart-contents" href="<?php echo wc_get_cart_url(); ?>" title="<?php _e( 'View your shopping cart','tesseract' ); ?>"><?php echo sprintf ('%d item', '%d items', WC()->cart->get_cart_contents_count() , WC()->cart->get_cart_contents_count() ); ?> - <?php echo WC()->cart->get_cart_total(); ?></a>
						</span>					
					<?php
					}
					else
					...
					case 'currency':
						if ( is_plugin_active( 'woocommerce/woocommerce.php' ) )
						{
							if(get_option('tesseract_selected_currency',true) && get_option('tesseract_selected_currency',true) != 1)
							{
								$currency_code_1 = get_option('tesseract_selected_currency',true);
							}
							else
							{
								$currency_code_1 = get_woocommerce_currency();
								//echo "----------------------".get_woocommerce_currency();
							}
							$currencies = get_option('wc_tesseract_currency_title',true);
							//echo "<pre>"; print_r($currencies); echo "</pre>";
							if($currencies != NULL){
								?>
								<span><select onchange='javascript:location.href = this.value;'>
								<?php
								foreach ($currencies as $currency) {
									?>
									<option <?php if($currency_code_1 == $currency ){ echo 'selected';} ?> value="<?php echo esc_url(get_the_permalink())?>/?currency=<?php echo $currency; ?>"><?php echo $currency; ?></option>
									<?php
								}
								echo "</select></span>";
							}
					...
					case 'searchcurrency':
						if ( is_plugin_active( 'woocommerce/woocommerce.php' ) )
						{
							...
							foreach ($currencies as $currency) {
								?>
								<option <?php if($currency_code_1 == $currency ){ echo 'selected';} ?> value="<?php echo $current_url; ?>/?currency=<?php echo $currency; ?>"><?php echo $currency; ?></option>
								<?php
							}
							echo "</select></span>";
							}
					...
					case 'woolinks':
						...
						{
							?>	
							<span style="color:<?php echo $tesseract_header_upper_text_color;?>;">
								<?php if ( is_user_logged_in() ) { ?>
								 	<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('My Account','tesseract'); ?>"><?php _e('My Account','tesseract'); ?></a> or 
								 	<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" href="<?php echo site_url();?>/logout"><?php _e('Logout','tesseract'); ?></a>
								 <?php } 
								 	...
								 	<a style="color:<?php echo $tesseract_header_upper_text_color;?>;" href="<?php echo get_permalink( get_option('woocommerce_myaccount_page_id') ); ?>" title="<?php _e('Login / Register','tesseract'); ?>"><?php _e('Login / Register','tesseract'); ?></a> or
								 	<a href="<?php echo $woocommerce->cart->get_checkout_url();?>" title="<?php _e('Checkout','tesseract'); ?>"><?php _e('Checkout','tesseract'); ?></a>
								<?php } ?>
							</span>
							<?php
								if(get_option('tesseract_selected_currency',true) && get_option('tesseract_selected_currency',true) != 1)
