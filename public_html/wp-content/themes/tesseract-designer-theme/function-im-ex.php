...
class Tesseract_Import_Export{
	...
	public function __construct() {
		if(isset($_POST['tesseract_im_ex']))
		{
			if($_POST['tesseract_im_ex'] == 1){
				...
				{
					$this->import($_POST['im_type']);
				}
				if($_POST['submit'] == 'Export')
				{
					$this->export($_POST['ex_type']);
				}
			}
		}
				...
				<form method="post" action="" novalidate="novalidate" name="tesseract_export" id="tesseract_export">
					<table class="form-table">
						<tr>
							...
							<td>
								<select name="ex_type">
									<option value="post">Post Data(Banner, Collection)</option>
									<option value="settings">Theme Settings Data</option>
								</select>
							</td>
						</tr>
						<tr>
    		...
    		<div class="wrap">
				...
				<form method="post" action="" novalidate="novalidate" id="tesseract_import" name="tesseract_import" enctype="multipart/form-data">
					<table class="form-table">
						...
						<tr><th scope="row">File:</th><td><input type="file" name="import_file" /><p>Only CSV format file is accepted.</p></td></tr>
						<tr>
							<td>
								<input type="hidden" name="tesseract_im_ex" value="1" />
    ...
    public function import($im_type)
    {
    	...
    	if($im_type == 'post')
    	{
    		while (($data = fgetcsv($handle, 1000, ",")) !== FALSE)
    		{
	    		if($count ==1 )
			 	{
			 		if( $data[0]!='name' || $data[1]!='title' || $data[2]!='type' || $data[3]!='image' )
			 		{
			 			add_action('admin_notices', array($this,'error_msg'	));
						return;
			 		}
    		 	...
    		 	{
				 	/*if($data[9] != ''){
					 	$cat_ID = get_cat_ID( $data[9] );

						//If it doesn't exist create new category
						if($cat_ID == 0) {
						$cat_name = array('cat_name' => $data[9]);
					...
					//Get ID of category again incase a new one has been created
				 	...
				 	$post_arr  = array('post_name'=>$data[0],'post_title'=>$data[1],'post_type'=>$data[2],'post_status'=>'publish' );
				 	//echo "<pre>"; print_r($data[11]); print_r($data[12]); print_r($data[13]); print_r($data[14]); echo "</pre>";
					//die;
					$postId    = wp_insert_post($post_arr);
					if($data[2] == 'page' && $data[5] != '')
					{
						update_post_meta($postId,'_wp_page_template',$data[5]);
					}
					//echo "--------------------------------PostId: ".$postId.'<br/>------------------------------------------------';
					if( $data[3] != '' )
					{
						$this->upload_image($data[3],$postId, $data[2]);
					}
					/*if( $data[7] != ''){
						set_post_format( $postId, $data[7] );
					}
					if( $data[8] != ''){
						wp_set_post_tags( $postId, $data[8], true );
					}*/
    ...
    public function export($ex_type)
    {
    	$filename = $ex_type."_data_".date('Y-m-d')."_".date('H:i:s').".csv";
		...
		if($ex_type == 'settings')
		{
			...
			$data[] = array('Header Background Color(Lower Part)', 'tesseract_header_colors_bck_color', (get_theme_mod('tesseract_header_colors_bck_color')) ? get_theme_mod('tesseract_header_colors_bck_color') : '#ffffff');
			...
			// $data[] = array('Submenu Text Color', 'sub_menu_color', (get_theme_mod('sub_menu_color')) ? get_theme_mod('sub_menu_color') : '#f1f1f1');
		...
		if($ex_type == 'post')
		{
			$header = array('name','title','type','image');
			header('Content-type: application/csv');
			header('Content-Disposition: attachment; filename='.$filename);
			fputcsv($fp, $header);
			/* Custom Posts */ 
			$tesseract_args = array( 'numberposts' => -1, 'post_status' => 'publish', 'post_type' => array('banner','collection')); 
			$tesseract_posts= get_posts( $tesseract_args );
			if ($tesseract_posts) 
			{
			    $data   = array();
			    foreach ( $tesseract_posts as $post ) {
			        // setup_postdata($post);
			        // the_title();
			        // the_excerpt();
				    ...
				    if(count($cat_arr) != 0)
			        {
				        ...
				        foreach($cat_arr as $cat){
				        	$cat_str .= get_cat_name($cat);
				        	if(count($cat_arr) != $count_cat)
				        	{
				        		$cat_str .= ',';
				        	}
				        	$count_cat++;
			        ...
			        $data[] =  array($post->post_name,$post->post_title,$post->post_type,$image_src);
